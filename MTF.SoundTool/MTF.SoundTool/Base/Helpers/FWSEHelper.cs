        ...
        public static bool ValidadeFWSEFile(string FilePath, string FileName)
        {
            using (FileStream FS = new FileStream(FilePath, FileMode.Open))
            {
                using (BinaryReader BR = new BinaryReader(FS))
                {
                    ...
                    if (Version != (int)FWSEVersion.RE5)
                    {
                        XtraMessageBox.Show($"Error reading {FileName}: Unsupported file version, only RE5 version is currently supported.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return false;
                    }
